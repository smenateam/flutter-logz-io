...
abstract class BaseLogSender extends BaseLogAppender {
  ...
  Future<void> log(DateTime time, Object line, Map<String, String>? TESTMESSAGE, Map<String, String> lineLabels) {
    return _logEvent(LogEntry(ts: time, logMessage: TESTMESSAGE!, line: line, lineLabels: lineLabels));
  }
  ...
  Future<void> _triggerSendLogEvents() =>
      Future(() {
        ...
        return _sendQueue.triggerJobRuns().then((val) {
          _logger.finest({'test': 'test'}, 'Sent log jobs: $val');
          return null;
        });
      });
  ...
  void handle(LogRecord record) {
    final Object message = formatter.format(record);
    final lineLabels = {
      'LOGGING_LEVEL': record.level.name,
    ...
    log(record.time, message, record.TESTMESSAGE!, lineLabels);
  }
...
class LogEntry {
  LogEntry({required this.ts, required this.line, this.logMessage, required this.lineLabels});
  ...
  String get tsFormatted => _dateFormat.format(ts);
}
...
class SimpleJobQueue {
  ...
  Future<int> triggerJobRuns() {
    if (_currentStream != null) {
      _logger.info({'test': 'test'}, 'Already running jobs. Ignoring trigger.');
      return Future.value(0);
    }
    _logger.finest({'test': 'test'}, 'Triggering Job Runs. ${_queue.length}');
...
//    final job = _queue.removeFirst();
    _currentStream = (() async* {
      ...
      successfulJobs++;
      _logger.finest({'test': 'test'},
          'Success job. remaining: ${_queue.length} - completed: $successfulJobs');
    }, onDone: () {
      _logger.finest({'test': 'test'}, 'All jobs done.');
      _errorCount = 0;
      _lastError = null;
      ...
      _logger.warning({'test': 'test'}, 'Error while executing job', error, stackTrace);
      _errorCount++;
      ...
      if (_lastError!.difference(DateTime.now()).abs().compareTo(minWait) < 0) {
        _logger.finest({'test': 'test'}, 'There was an error. waiting at least $minWait');
        if (_queue.length > maxQueueSize) {
          _logger.finest({'test': 'test'}, 'clearing log buffer. ${_queue.length}');
          _queue.clear();
        }
      }
      return Future.value(null);
