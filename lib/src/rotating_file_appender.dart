...
class RotatingFileAppender extends BaseLogAppender {
  ...
  void handle(LogRecord record) {
    ...
    try {
      ...
      _logger.warning({'test': 'test'}, 'Error while writing log.', error, stackTrace);
      _closeAndFlush();
      // try once more.
      _getOpenOutputFileSink()..writeln(formatter.format(record));
    }
  ...
  Future<bool> _maybeRotate() async {
    ...
    try {
      try {
        ...
        _logger.warning({'test': 'test'}, 'Error while checking file legnth.', e, stackTrace);
        rethrow;
      }
      ...
      handle(LogRecord({'test': 'test'}, Level.INFO, 'Rotated log.', '_'));
      await flushFuture;
      return true;
    } finally {
  ...
  Future<void> _closeAndFlush() async {
    ...
    if (_outputFileSink != null) {
      try {
        ...
        _logger.warning({'test': 'test'}, 'Error while flushing, closing stream.', e, stackTrace);
      }
    }
  }
